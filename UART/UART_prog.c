...
void USART_voidIni(USART_t * PtrToUART)
{
	...
	else if((PtrToUART->stop_bits)==BIT_1)
	...
	else if ((PtrToUART->parity)==NO_PARITY)
	{
		CLR_BIT(USCRC_Value,UCSRC_UPM0);
...
/*ISR for Reciving  */
 ...
 u8 USART_u8RecivString(char * Copy_Sring)
 {
		u8 Local_u8ErrorState=OK;
		u8 Local_u8Counter=0;
		/*Check the NULL pointer  */
		if(Copy_Sring!=NULL)
			{
				while(Copy_Sring[Local_u8Counter]!='#')
					USART_voidReciveDataSync(&Copy_Sring[Local_u8Counter]);
					Local_u8Counter++;
			}
		else
			{
